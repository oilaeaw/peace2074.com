        ...
        <q-card flat bordered class="q-mb-md">
          <q-card-section>
            <div class="text-h6 q-mb-sm">
              <q-select v-model="loginType" :options="loginOptions" :label="t('login_method')" filled dense style="max-width: 200px" />
            </div>
            <q-form class="q-gutter-md" @submit.prevent="onSubmit" @reset="onReset">
              <q-input v-if="loginType === 'email'" v-model="loginPayload.email" type="email" :label="t('email')" autocomplete="username" :rules="[(val) => !!val || t('email_required')]" />
              <q-input v-else v-model="loginPayload.username" type="text" :label="t('username')" autocomplete="username" :rules="[(val) => !!val || t('username_required')]" />
              <q-input v-model="loginPayload.password" :type="showPassword ? 'text' : 'password'" :label="t('password')" autocomplete="current-password" :rules="[(val) => !!val || t('password_required')]">
                <template #append>
            ...
            <div class="q-mt-md">
              <q-btn class="q-mb-sm full-width" label="Login with Google" icon="fab fa-google" style="background: #fff; color: #4285f4; border: 1px solid #4285f4" @click="onGoogleLogin" />
              <q-btn class="full-width" label="Login with GitHub" icon="fab fa-github" style="background: #24292e; color: #fff" @click="onGithubLogin" />
            </div>
          </q-card-section>
        </q-card>
      </q-card-section>
