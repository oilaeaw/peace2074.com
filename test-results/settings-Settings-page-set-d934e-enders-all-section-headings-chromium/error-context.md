...
- Name: settings.spec.ts >> Settings page >> settings page renders all section headings
- Location: tests/settings.spec.ts:42:5
...
Locator:  getByText('Navigation', { exact: true }).first()
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for getByText('Navigation', { exact: true }).first()
    6 × locator resolved to <div data-v-cfff4234="" class="drawer-title text-subtitle1 text-weight-bold">Navigation</div>
