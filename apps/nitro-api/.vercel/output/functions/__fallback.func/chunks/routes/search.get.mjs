...
import { a as chaptersTr, d as chaptersHe, b as chaptersDe, c as chaptersRu } from '../_/he.mjs';
...
const chaptersMap = {
  ...
  tr: chaptersTr,
  ar: chaptersEn,
  // Arabic sura names are in the 'name' field of all chapter files
  it: chaptersEn
  // Italian fallback to English
};
const quranCache = quranData;
...
const search_get = defineEventHandler(async (event) => {
  ...
  try {
    ...
    }).select("slug title excerpt tags").limit(5).lean();
