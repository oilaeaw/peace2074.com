...
function normalizeAppUrl(url?: string) {
  ...
  try {
    const parsed = raw ? new URL(raw, appOrigin) : new URL(appOrigin)
...
const initialUrl = normalizeAppUrl(props.src)
const webViewSrc = ref(initialUrl)
...
const currentAppUrl = ref(initialUrl)
...
function reloadApp(url = appOrigin) {
  webViewSrc.value = normalizeAppUrl(url)
  webViewKey.value += 1
  updateLoadingState()
}
...
function handleNativeCallback(url: string) {
  ...
  if (authComplete === '1') {
    ...
    const urlObj = new URL(currentAppUrl.value || appOrigin)
...
watch(
  () => props.src,
  (value) => {
    const nextUrl = normalizeAppUrl(value)
    currentAppUrl.value = nextUrl

    if (nextUrl !== webViewSrc.value) {
      reloadApp(nextUrl)
    }
  }
)
